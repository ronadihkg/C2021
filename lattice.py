...
with open("LWE Ring Signature Time Analysis Long.txt", "w") as text_file:
    text_file.write("N Size\tSign\tVerify\tTotal\tTimes of Restart\n")
...
#    text_file.write("256 Sign\tVerify\tTotal\tTimes of Restart\n")
    for i in range (2):
        text_file.write("%d\t" % (2**(i+1)))
        #     Change range(1) for testing for multiple times.
        for j in range (1):
            text_file.write("%s\t%s\t%s\t%s\t" % (all_sign_time[i][j], 
                                                  all_verify_time[i][j], all_total_time[i][j], 
                                                  restart_counter[i][j]))
        text_file.write("\n")
